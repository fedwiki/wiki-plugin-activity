...
const bind = ($item, item) => {
  ...
  const display = (query, pages) => {
    ...
    const activityTitle = h('div', {style: {textAlign:'center', fontSize: 'small', color:'gray', marginTop: '14px'}}, header);
    ...
    for (const sites of pages) {
      if (((sites.length >= query.twins) || query.twins === 0) &&
        ...
        const pageURL = (sites.length === 1 && sites[0].site !== location.host) || !sites.some(i => i.site === location.host)
          ? wiki.site(sites[0].site).getURL(`/${sites[0].page.slug}.html`)
          : `/${sites[0].page.slug}.html`;

        const pageLink = h('a.internal', {
          href: pageURL,
          title: context,
        ...
        activityBody.push(h('div', {style: {clear: 'both'}, id: sites[0].page.slug}, [
